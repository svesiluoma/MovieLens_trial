...
```{r create_d, echo=FALSE, results = "hide"}
library(tidyverse)
library(caret)
library(dslabs)
...
The summary of the training set clearly shows that there are no NA values, so no need for pre-processing those in this data.
```{r summary}
summary(edx)
```

The rows in the data look like this
```{r head}
edx %>% head()
```
...
Also, when looking the amount of ratings people have used, it is clear that they have used integers much more than the half (.5) figures as can bee seen from the line chart below. 
```{r ratings_rates}
edx %>%
  group_by(rating) %>%
  summarize(count = n()) %>%
